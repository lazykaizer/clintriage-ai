...
def main():
    ...
    try:
        ...
        check("Returns 200", r.status_code == 200, f"Got {r.status_code}")
        tasks = r.json()
        check("Returns 5 tasks", len(tasks) == 5, f"Got {len(tasks)} tasks")
    ...
    # ─── Step: Task 1 (Binary Triage) ─────────────────────
    print("\n[5] Step — Task 1 (POST /step)")
    try:
        requests.post(f"{BASE}/reset", json={"task_id": 1}, timeout=10)
        r = requests.post(
            f"{BASE}/step",
            json={"task_id": 1, "triage_decision": "LEVEL_2", "reasoning": "Test reasoning"},
            timeout=10,
        )
        check("Returns 200", r.status_code == 200, f"Got {r.status_code}")
        data = r.json()
    ...
    # ─── Step: Task 5 (Edge Case) ─────────────────────────
    print("\n[9] Step — Task 5 (POST /step)")
    try:
        requests.post(f"{BASE}/reset", json={"task_id": 5}, timeout=10)
        r = requests.post(
            f"{BASE}/step",
            json={"task_id": 5, "triage_decision": "LEVEL_1", "reasoning": "Suspected hidden critical condition"},
            timeout=10,
        )
        check("Returns 200", r.status_code == 200, f"Got {r.status_code}")
        data = r.json()
