...
def grade_task1(agent_ranking: List[str], patients: list) -> Tuple[float, str]:
    ...
    if not agent_ranking or len(agent_ranking) != 2:
        return 0.01, "Invalid ranking provided. Must rank 2 patients."
    ...
    if agent_ranking == truth_ids:
        return _nudge_score(1.0), "Perfect priority order for 2 patients."
    else:
        return _nudge_score(0.0), f"Incorrect priority order. Ground truth: {truth_ids}"
...
def grade_task2(agent_ranking: List[str], patients: list) -> Tuple[float, str]:
    ...
    if not agent_ranking or len(agent_ranking) != len(patients):
        return 0.01, f"Invalid ranking length. Expected {len(patients)}."
    ...
    if is_sorted:
        return _nudge_score(1.0), "Perfect clinical priority order (levels are correctly prioritized)."
    ...
    if truth_map.get(agent_ranking[0]) == min_level:
        return _nudge_score(0.6), f"Safely identified a most-critical patient (Level {min_level}) as #1."
    ...
    if correct_positions > 0:
        return _nudge_score(0.3), f"Partial overlap — {correct_positions} clinical level(s) in correct position."

    return 0.01, "Incorrect priority order."
...
def grade_task4_programmatic(icu_selection: List[str], patients: list) -> Tuple[float, str]:
    ...
    if not icu_selection:
        return 0.01, "No ICU selection provided."
