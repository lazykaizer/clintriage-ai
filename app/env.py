...
class ClinTriageEnv:
    def __init__(self):
        ...
        self.edge_cases   = [c for c in all_cases if c.get("is_edge_case")]
    ...
    def reset(self, task_id: int) -> dict:
        ...
        if task_id == 1:
            patient = random.choice(self.normal_cases)
            self.current_patients = [patient]
            return {
                "patient": self._clean(patient),
                "instructions": (
                    "Classify this patient as CRITICAL (LEVEL_1 or LEVEL_2) or "
                    "NON-CRITICAL (LEVEL_3, LEVEL_4, LEVEL_5). "
                    "Return JSON: {\"triage_decision\": \"LEVEL_X\", \"reasoning\": \"your explanation\"}"
                )
            }
            ...
            patient = random.choice(self.edge_cases)
            self.current_patients = [patient]
            return {
                "patient": self._clean(patient),
                "instructions": (
                    "Triage this patient carefully. Be alert for hidden serious conditions "
                    "that may not be immediately obvious from the chief complaint. "
                    "Return JSON: {\"triage_decision\": \"LEVEL_X\", \"reasoning\": \"your explanation\"}"
                )
            }
